...
Apache Spark’s basic component is made up of RDD (Resilient Distributed Dataset) and it is processed in memory and based on lazy execution in a distributed environment, it is capable of handling streaming data.
HBase is preferred to be used is this scenario where the datastore tables store the aggregate views for hour of day, day of week of each locations price and its surge in price is calculated from previous data. 
